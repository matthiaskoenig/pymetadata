...
**How do you ship such a study as one unit?** The COMBINE archive (OMEX) answers
this. It is a ZIP container with a `manifest.xml` listing every file and, for
each file, the format it is in as an identifiers.org URI. `pymetadata` reads,
writes and validates these archives, see [COMBINE archives](omex.qmd).
