...
- logging follows the convention for libraries now: `pymetadata` no longer adds
  ...
  and examples
- `log.get_logger()` removed: it only returned `logging.getLogger(name)`, so the
  modules of the package now use the standard library directly. Code which
  called it changes from `log.get_logger(__name__)` to
  `logging.getLogger(__name__)`; `pymetadata.log` keeps
  `enable_rich_logging()` and `PACKAGE_LOGGER`
...
- cached responses expire now, instead of being kept forever: the OLS, ChEBI
  and UniChem responses after 30 days (`CACHE_DURATION_ONTOLOGY`), since they
...
- license metadata follows PEP 639, i.e., `license = "MIT"` with
...
- root configuration cleaned up: `.gitignore` lost the entries of tools which
  are not used (setuptools `.eggs/` and egg-info, `cover/`, `.benchmark`,
  `wheels`, `.Rhistory`, paths of the examples which moved out of the package),
  the unused `pytest-raises` dependency and its `raises` marker are gone, as is
  the `install` tox environment which nothing referenced, and the pytest
  configuration moved from `tox.ini` to `[tool.pytest.ini_options]` in
  `pyproject.toml`
