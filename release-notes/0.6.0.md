...
- ontology terms carry the information of the ontology release: a member of
  `SBO`, `KISAO` or `PBPKO` provides `label`, `definition`, `synonyms`,
  `deprecated`, `curie` and `url`, `term` gives the complete `OntologyTerm` and
  `get_term` does the same for a term given as a string. The behaviour of the
  enums moved into `OntologyEnum` in the new `pymetadata.ontologies.term`, the
  generated modules only carry the data. The interface is unchanged: the
  members, the `str` behaviour, `get_name` and `validate` work as before, only
  `repr` shows the label now, e.g., `<SBO.SBO_0000247: 'simple chemical'>`
